...
// Uses a SEPARATE Firebase instance for the bot's Firestore
// ============================================================

// ─── Bot's Firebase Config (ascend-hub-discord project) ─────
const botFirebaseConfig = {
  apiKey:            "AIzaSyDkM6YFfaaL_273QFYFJ_siuz4_Qbsyplg",
  authDomain:        "ascend-hub-discord.firebaseapp.com",
  projectId:         "ascend-hub-discord",
  storageBucket:     "ascend-hub-discord.firebasestorage.app",
  messagingSenderId: "200063856024",
  appId:             "1:200063856024:web:f16882583efe5b0e5a4fed"
};

// Initialize a SECOND Firebase app just for the bot's Firestore
// Name it 'botApp' so it doesn't clash with the main RankForge app
let botDb;
(function initBotFirebase() {
  try {
    // Check if already initialized to avoid duplicate app error
    const existingApp = firebase.apps.find(a => a.name === 'botApp');
    const botApp = existingApp || firebase.initializeApp(botFirebaseConfig, 'botApp');
    botDb = botApp.firestore();
    console.log('✅ Bot Firebase initialized');
  } catch (e) {
    console.error('Bot Firebase init error:', e);
  }
})();
...
// ─── Load Discord user from session ──────────────────────────
  ...
  if (!isLoggedIn) {
    page.innerHTML = `
      ...
      <div class="card" style="max-width:480px;margin:40px auto;text-align:center;padding:48px 40px">
        ...
        <p class="text-xs text-muted" style="margin-top:16px">We only request your username. No messages are accessed.</p>
      </div>
    `;
    return;
  }
  ...
  page.innerHTML = `
    <div class="page-header" style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:16px">
      ...
      <div style="display:flex;align-items:center;gap:12px;background:var(--bg-surface);border:1px solid var(--border-soft);border-radius:12px;padding:10px 16px">
        ...
        <div>
          <div style="font-weight:600;font-size:0.9rem;color:var(--text-primary)">${escapeHtml(u.global_name || u.username)}</div>
    ...
    <div id="dc-challenges-container">
      <div class="card" style="text-align:center;padding:40px">
        ...
        <p class="text-muted text-sm" style="margin-top:12px">Loading challenge data from bot…</p>
      </div>
    </div>

    <div id="dc-ended-container" style="margin-top:22px"></div>
  `;
...
// ─── Load from BOT's Firestore (botDb) ───────────────────────
async function loadDiscordChallengeData(discordUserId) {
  if (!botDb) {
    document.getElementById('dc-challenges-container').innerHTML = `
      <div class="card" style="text-align:center;padding:40px">
        <div style="color:var(--red);font-size:2rem;margin-bottom:12px">⚠️</div>
        <h3>Bot database not connected</h3>
        <p class="text-muted text-sm">Could not initialize bot Firebase. Check console.</p>
      </div>`;
    return;
  }
  ...
  try {
    const [activeSnap, endedSnap] = await Promise.all([
      botDb.collection('challenges').where('status', '==', 'active').get(),
      botDb.collection('challenges').where('status', '==', 'ended')
    ...
    document.getElementById('dc-challenges-container').innerHTML = `
      <div class="card" style="text-align:center;padding:40px">
        ...
        <button class="btn btn-secondary btn-sm" style="margin-top:16px"
          onclick="loadDiscordChallengeData('${discordUserId}')">Retry</button>
      </div>`;
  }
}
    ...
    const topRows = participants.slice(0, 10).map((p, i) => {
      ...
      return `
        <tr style="${isMe ? 'background:rgba(255,209,0,0.06)' : ''}">
          ...
          <td><span class="badge ${isMe ? 'badge-yellow' : i < 3 ? 'badge-cyan' : 'badge-muted'}">${isMe ? 'You' : i < 3 ? 'Top 3' : 'Active'}</span></td>
        </tr>`;
    }).join('');
        ...
        <div class="card-header" style="flex-wrap:wrap;gap:12px">
          <div>
            <div class="card-title">
              <svg width="16" height="16" viewBox="0 0 71 55" fill="#5865F2" style="flex-shrink:0">
                <path d="M60.1 4.9A58.5 58.5 0 0 0 45.5.9a.2.2 0 0 0-.2.1 40.7 40.7 0 0 0-1.8 3.7 54 54 0 0 0-16.2 0A37.6 37.6 0 0 0 25.5 1a.2.2 0 0 0-.2-.1A58.3 58.3 0 0 0 10.7 4.9a.2.2 0 0 0-.1.1C1.5 18.1-1 30.9.4 43.5a.2.2 0 0 0 .1.1 58.8 58.8 0 0 0 17.7 8.9.2.2 0 0 0 .2-.1 42 42 0 0 0 3.6-5.9.2.2 0 0 0-.1-.3 38.7 38.7 0 0 1-5.5-2.6.2.2 0 0 1 0-.4l1.1-.8a.2.2 0 0 1 .2 0c11.5 5.3 24 5.3 35.4 0a.2.2 0 0 1 .2 0l1.1.8a.2.2 0 0 1 0 .4 36.2 36.2 0 0 1-5.5 2.6.2.2 0 0 0-.1.3 47.1 47.1 0 0 0 3.6 5.9.2.2 0 0 0 .2.1 58.6 58.6 0 0 0 17.8-8.9.2.2 0 0 0 .1-.1c1.7-15-2.8-27.7-11.9-39.1a.2.2 0 0 0-.1-.2zM23.7 36c-3.5 0-6.4-3.2-6.4-7.1s2.8-7.1 6.4-7.1c3.6 0 6.5 3.2 6.4 7.1 0 3.9-2.8 7.1-6.4 7.1zm23.6 0c-3.5 0-6.4-3.2-6.4-7.1s2.8-7.1 6.4-7.1c3.6 0 6.5 3.2 6.4 7.1 0 3.9-2.8 7.1-6.4 7.1z"/>
              </svg>
  ...
  const rows = challenges.map(ch => {
    ...
    return `
      <tr>
        ...
        <td>${winner ? `<span class="mono" style="font-size:0.8rem">🥇 ${((winner[1].studyMinutes||0)/60).toFixed(1)}h</span>` : '—'}</td>
    ...
    <div class="card">
      ...
      <div class="table-wrap">
        <table>
          <thead>
            <tr>
              <th>Challenge</th><th>Ended</th><th>Participants</th><th>Winner</th><th>Your Rank</th>
            </tr>
          </thead>
          <tbody>${rows}</tbody>
        </table>
...
function renderDiscordStats(active, ended, myDiscordId) {
  ...
  [...active, ...ended].forEach(ch => {
    const myEntry = ch.participants?.[myDiscordId];
    if (!myEntry) return;
    totalMinutes += myEntry.studyMinutes || 0;
    const sorted = Object.entries(ch.participants || {})
      .sort((a, b) => (b[1].studyMinutes || 0) - (a[1].studyMinutes || 0));
    const rank = sorted.findIndex(p => p[0] === myDiscordId) + 1;
    if (rank > 0 && rank < bestRank) bestRank = rank;
  });

  const el = id => document.getElementById(id);
  if (el('dc-totalHours'))       el('dc-totalHours').textContent       = (totalMinutes / 60).toFixed(1) + 'h';
  if (el('dc-activeChallenges')) el('dc-activeChallenges').textContent = active.length;
  if (el('dc-bestRank'))         el('dc-bestRank').textContent         = bestRank === Infinity ? '—' : '#' + bestRank;
}
...
// ─── Auto-navigate if returning from Discord OAuth ────────────
(function () {
