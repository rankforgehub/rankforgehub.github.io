...
function renderDashboardChart() {
  ...
  const textColor = isDark ? '#6b7a8e' : '#94a3b8';
  ...
  App.charts.weekly = new Chart(canvas, {
    ...
    data: {
      ...
      datasets: [
        ...
        {
          label: 'Study Hours',
          data: [2.5, 4, 3, 5, 3.5, 6, 4.5],
          type: 'line',
          borderColor: '#00e6dc',
          backgroundColor: 'rgba(0,230,220,0.07)',
          borderWidth: 2,
          pointRadius: 4,
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          backgroundColor: isDark ? '#151c2c' : '#fff',
          borderColor: isDark ? '#1e2a3d' : '#e2e8f0',
          borderWidth: 1,
...
function getChartColors() {
  ...
  return {
    ...
    text: isDark ? '#6b7a8e' : '#94a3b8',
    surface: isDark ? '#151c2c' : '#fff',
    border: isDark ? '#1e2a3d' : '#e2e8f0',
  ...
  App.charts.consistency = new Chart(canvas, {
    ...
    data: {
      ...
      datasets: [{
        ...
        backgroundColor: 'rgba(0,230,220,0.05)',
        borderColor: '#00e6dc',
        borderWidth: 1.5,
        pointBackgroundColor: '#00e6dc',
        pointRadius: 3,
    ...
    options: {
      ...
      scales: {
        r: {
          grid: { color: c.grid },
          ticks: { color: c.text, backdropColor: 'transparent', font: { family: 'JetBrains Mono', size: 9 } },
  ...
  App.charts.subject = new Chart(canvas, {
    ...
    options: {
      ...
      plugins: {
        legend: {
          position: 'bottom',
          labels: { color: c.text, font: { family: 'JetBrains Mono', size: 11 }, padding: 16 }
        }
      }
    }
  });
...
function renderPointsHistoryChart() {
  ...
  let cumsum = 0;
  const cumData = Array.from({length: 30}, () => Math.floor(Math.random() * 80 + 20)).map(v => cumsum += v);
  ...
  App.charts.pointsHistory = new Chart(canvas, {
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: { backgroundColor: c.surface, borderColor: c.border, borderWidth: 1, titleColor: c.primary, bodyColor: c.text, cornerRadius: 8, padding: 10 }
      },
      scales: {
        x: { grid: { color: c.grid }, ticks: { color: c.text, font: { size: 10 } } },
        y: { grid: { color: c.grid }, ticks: { color: c.text, font: { size: 10, family: 'JetBrains Mono' } } }
      }
    }
  });
}
